from warcio.cli import Recompressor
from warcio.cli import Checker
...
class WarcChecker:
    ...
    def recompress_warc_file_in_correct_format(filename):
        ...
        """
        recompression_result_file_location = WarcChecker.get_new_recompression_file_location(filename)
    ...
    def get_new_recompression_file_location(filename):
        ...
        match = WarcChecker.WARC_FILE_REGEX.match(file_basename)
        filename = match.group(1)
        extension = match.group(2)

        recompression_file_name = filename + WarcChecker.WARC_RECOMPRESSED_ENDING + extension
